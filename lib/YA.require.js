...
(function (YA) {
    ...
    })(MemberAccessorTargetType || (MemberAccessorTargetType = {}));
    ...
    function attach(elem, eventId, listener) {
        if (elem.addEventListener) {
            elem.addEventListener(eventId, listener, false);
        }
        else if (elem.attachEvent) {
            elem.attatchEvent('on' + eventId, listener);
        }
    }
    YA.attach = attach;
    function detech(elem, eventId, listener) {
        if (elem.removeEventListener) {
            elem.removeEventListener(eventId, listener, false);
        }
        else if (elem.detechEvent) {
            elem.detechEvent('on' + eventId, listener);
        }
    }
    YA.detech = detech;
    ...
    function removeClass(elem, cls) {
        ...
        var prev = clsnames.substr(0, at);
    }
    YA.removeClass = removeClass;
    var ValueAttribute = /** @class */ (function (_super) {
        __extends(ValueAttribute, _super);
        function ValueAttribute(name, value, velem) {
            return _super.call(this, name, value, velem) || this;
        }
        return ValueAttribute;
    }(VAttribute));
    YA.ValueAttribute = ValueAttribute;
